...
def gather_facts() -> dict:
    ...
    horizonte = overall.get('180d') if (overall.get('180d') or {}).get('count') else overall.get('90d')
    ...
    return {
        ...
        'rendimiento_sistema': {
            'horizonte': '180d' if (overall.get('180d') or {}).get('count') else '90d',
            'acierto_pct': (horizonte or {}).get('win_rate'),
            'retorno_medio': (horizonte or {}).get('avg_return'),
            'n': (horizonte or {}).get('count'),
        },
    }
...
1. Qué comprar hoy. Si la lista de comprables viene vacía, la PRIMERA línea es que
   hoy no hay nada, sin rodeos ni consuelo.
...
Reglas de escritura:
- Español, tono directo de colega que sabe del tema. Nada de "¡Buenos días!" ni emojis
  decorativos ni disclaimers.
- Máximo 12 líneas en total. Es un móvil.
- Frases cortas. Un dato por frase.
- HTML de Telegram para dar énfasis: <b>negrita</b>. Nada de markdown.
- Si algo está barato pero cayendo, dilo así: barato no es comprable todavía."""
