...
from upside_triangulation import add_upside_triangulation
...
def calculate_european_value_scores(df: pd.DataFrame, market_regime: dict) -> pd.DataFrame:
    ...
    # ─── VALUATION CHECK: banda canónica de value_bands, no una propia ───
    # Antes esta lista solo rechazaba upside<0 y premiaba R:R>=3.0 (equivalente
    # a upside>=24%) con el bonus MÁS alto — exactamente el tramo que el
    # backtest de la lista US demostró que es un value trap (0% win, -8.28%
    # medio en 55 señales reales). SAP.DE con upside +20.4% se libraba por poco,
    # pero la lista entera llevaba meses puntuando con la banda ya desmentida.
    if 'analyst_upside_pct' in df.columns:
        ...
        if value_trap.sum() > 0:
            print(f"   Rejected (upside >= {UPSIDE_HARD_REJECT:.0f}% — value trap): {value_trap.sum()} tickers")
            df.loc[value_trap, 'value_score'] = 0.0

        df['upside_bonus'] = 0.0
        mask_golden = _up.notna() & (_up >= UPSIDE_MIN) & (_up < UPSIDE_GOLDEN_MAX)
        mask_mild   = _up.notna() & (_up >= 0) & (_up < UPSIDE_MIN)
        mask_trap   = _up.notna() & (_up >= UPSIDE_HARD_REJECT)
        ...
        print(f"   Upside tiers: {int(mask_golden.sum())} golden [{UPSIDE_MIN:.0f},{UPSIDE_GOLDEN_MAX:.0f}) (+5pts) · "
              f"{int(mask_trap.sum())} trap >={UPSIDE_HARD_REJECT:.0f}% (-5pts)")
        df.drop(columns=['upside_bonus'], inplace=True, errors='ignore')

    # ─── Risk/Reward ratio (informativo — se sigue publicando la columna,
    # pero ya no puntúa: el bonus vivía en el tramo que el backtest desmiente) ──
    if 'analyst_upside_pct' in df.columns:
        stop_loss_pct = 8.0
        ...
        print(f"   Risk/Reward (informativo): {(df['risk_reward_ratio'] >= 2.0).sum()} tickers with R:R >= 2")
    ...
    # Triangulación upside analista vs DCF/P·E propios — antes solo se
    # calculaba para US y las 36 filas europeas salían con
    # upside_triangulated_pct / upside_divergence vacíos. Va antes del corte
    # de calidad para que quien lea el CSV vea, junto al score, si el
    # descuento del analista está o no respaldado por los modelos propios.
    df = add_upside_triangulation(df)

    # ─── Filter: mismo corte que la lista US (value_bands.VALUE_SCORE_MIN) ───
    # Antes era 25 hardcodeado aquí; US usa 30. El mismo criterio de calidad
    # debe valer igual a los dos lados del Atlántico.
    qualified = df[df['value_score'] >= VALUE_SCORE_MIN].copy()
    qualified = qualified.sort_values('value_score', ascending=False)

    print(f"\nRESULTADOS:")
    print(f"   Total scored: {len(df)}")
    print(f"   Qualified (score >= {VALUE_SCORE_MIN:.0f}): {len(qualified)}")
    if len(qualified) > 0:
