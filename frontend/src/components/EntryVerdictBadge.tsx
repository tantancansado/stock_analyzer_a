...
export default function EntryVerdictBadge({ verdict, compact = false, className }: Readonly<Props>) {
  ...
  if (!verdict) {
    ...
    return (
      <span className={cn('inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-micro font-bold border bg-muted/10 text-muted-foreground border-border/20', className)}>
        <HelpCircle size={12} /> ?
      </span>
    )
  }
        ...
        <div className="absolute z-50 top-full left-0 mt-1 w-72 rounded-lg border border-border shadow-xl p-3 text-xs space-y-2" style={{ backgroundColor: 'var(--background)' }}>
          <div className="flex items-center gap-2 pb-1.5 border-b border-border/30">
            ...
            {verdict.confidence != null && (
              <span className="text-micro text-muted-foreground tabular-nums ml-auto">conf {verdict.confidence}</span>
            )}
            {verdict.source === 'ai' && (
