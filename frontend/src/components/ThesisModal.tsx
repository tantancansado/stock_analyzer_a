...
function Chip({ label, value, color }: { label: string; value: string; color?: string }) {
  const bg =
    color?.includes('emerald') ? 'bg-emerald-500/10 border-emerald-500/25' :
    color?.includes('red')     ? 'bg-red-500/10 border-red-500/20'         :
    color?.includes('amber')   ? 'bg-amber-500/10 border-amber-500/20'     :
    color?.includes('purple')  ? 'bg-purple-500/10 border-purple-500/20'   :
    color?.includes('primary') ? 'bg-primary/10 border-primary/25'         :
                                 'bg-muted/30 border-border/30'
  return (
    <div className={`flex flex-col items-center gap-0.5 px-3 py-2.5 rounded-xl border min-w-[60px] ${bg}`}>
      <span className="text-[0.52rem] font-bold uppercase tracking-widest text-muted-foreground/50">{label}</span>
  ...
  return (
    <>
      ...
      <div
        ...
        aria-modal="true"
      >
        <div className="relative w-full sm:max-w-2xl bg-[hsl(var(--card))] border border-border/60 shadow-2xl rounded-t-2xl sm:rounded-2xl flex flex-col max-h-[92dvh] sm:max-h-[88dvh] modal-enter overflow-hidden">
          {/* Top gradient accent */}
          <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-primary/40 via-primary to-purple-500/60 z-10" />
          {/* Handle (mobile) */}
