import { TriangleAlert, Check, X, Minus, type LucideIcon } from 'lucide-react'
interface Props {
  verdict?: string | null
...
function styleFor(verdict: string): { cls: string; icon: LucideIcon } {
  const v = verdict.toUpperCase()
  if (v.startsWith('UNRELIABLE')) {
    return { cls: 'bg-muted/30 text-muted-foreground border-border/40', icon: TriangleAlert }
  }
  if (v.startsWith('RELIABLE/BUY')) {
    return { cls: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30', icon: Check }
  }
