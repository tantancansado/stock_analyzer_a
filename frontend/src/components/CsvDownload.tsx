...
export default function CsvDownload({ dataset, label, className }: CsvDownloadProps) {
  return (
    <button
      ...
      className={`text-mini px-3 py-1 rounded border border-border/50 text-muted-foreground hover:text-foreground hover:border-primary transition-colors ${className ?? ''}`}
    >
      ↓ {label ?? 'CSV'}
    </button>
  )
