...
export default function BacktestResults() {
  ...
  const cumColor = cumFinal >= 0 ? 'var(--success)' : 'var(--danger)'
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={regimeStats} layout="vertical" margin={{ top: 0, right: 8, left: 0, bottom: 0 }}>
                ...
                <Bar dataKey="avg" radius={[0, 4, 4, 0]}>
                  {regimeStats.map((r, i) => (
                    <Cell key={i} fill={r.avg >= 0 ? 'var(--success)' : 'var(--danger)'} fillOpacity={0.8} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
              ...
              <BarChart data={scoreQuartiles} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="avg" radius={[4, 4, 0, 0]}>
                  {scoreQuartiles.map((q, i) => (
                    <Cell key={i} fill={q.avg! >= 0 ? 'var(--success)' : 'var(--danger)'} fillOpacity={0.7 + i * 0.075} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <CardContent className="p-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={sectorStats} layout="vertical" margin={{ top: 0, right: 8, left: 10, bottom: 0 }}>
                ...
                <Bar dataKey="avg" radius={[0, 4, 4, 0]}>
                  {sectorStats.map((s, i) => (
                    <Cell key={i} fill={s.avg >= 0 ? 'var(--success)' : 'var(--danger)'} fillOpacity={0.75} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
