...
function ActionBadge({ action }: { action: StrategyAction }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-mini font-bold uppercase tracking-wider ${meta.bg}`}>
  ...
  return (
    <Card className="liquid-glass overflow-clip">
      <CardContent className="p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 mb-4">
          {s.trim_at_price && s.trim_pct ? (
            <div className="rounded-lg border border-amber-500/25 bg-amber-500/5 px-3 py-2.5">
              ...
              {s.trim_reason && (
                <div className="mt-1.5 text-mini leading-snug text-muted-foreground">{s.trim_reason}</div>
              )}
            </div>
          ) : (
          ...
          {s.add_at_price && s.add_pct ? (
            <div className="rounded-lg border border-emerald-500/25 bg-emerald-500/5 px-3 py-2.5">
              ...
              {s.add_reason && (
                <div className="mt-1.5 text-mini leading-snug text-muted-foreground">{s.add_reason}</div>
              )}
            </div>
          ) : (
            <div className="rounded-lg border border-border/20 bg-muted/5 px-3 py-2.5 opacity-50">
        ...
        {/* Thesis + next check */}
        {s.thesis_short && (
          <div className="mb-3 text-apoyo leading-relaxed text-muted-foreground italic">
            “{s.thesis_short}”
          </div>
        )}
      ...
      <div className="mb-5 flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {(['ADD', 'TRIM', 'HOLD', 'WATCH', 'EXIT'] as StrategyAction[]).map(action => {
            ...
            return (
              <div key={action} className={`text-mini font-bold px-3 py-1.5 rounded-full border ${meta.bg}`}>
                {n} {meta.label}
              </div>
            )
          })}
