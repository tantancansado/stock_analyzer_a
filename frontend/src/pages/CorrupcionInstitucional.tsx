...
function StrengthBadge({ strength }: { strength: string }) {
  return (
    <span className={cn('text-micro font-semibold px-1.5 py-0.5 rounded border', STRENGTH_STYLE[strength] ?? STRENGTH_STYLE.LOW)}>
      {strength}
    </span>
  )
}
        ...
        {TABS.map(tab => {
          ...
          return (
            ...
            >
              ...
              {count > 0 && (
                <span className={cn('text-micro font-bold px-1 py-0.5 rounded', activeTab === tab.id ? 'bg-muted' : 'bg-muted/50')}>
                  {count}
                </span>
              )}
            </button>
