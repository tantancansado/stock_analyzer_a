...
def run_filter():
    ...
    filtered = merged[
        ...
        (merged["Debt/Equity"].astype(float) < 2) &
        ((merged["ROE"].astype(str) != "nan") & (merged["ROE"].astype(float) > 0)) &
        ((merged["Gross Margin"].astype(str) != "nan") & (merged["Gross Margin"].astype(float) > 0))
    ].copy()
